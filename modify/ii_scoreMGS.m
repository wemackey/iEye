function [ii_trial,ii_cfg] = ii_scoreMGS(ii_data,ii_cfg,ii_sacc,targ_coords,resp_epoch,fix_epoch,excl_criteria,save_chans,score_mode)
...
%elseif % if targ_coords already defined, first check to be sure it's either some multiple of n_trials long, or same size as # of channels used for ii_sacc
    % TODO:
end
...
for ee = 1:length(excl_fields)
    if ~ismember(fields(excl_criteria),excl_fields{ee})
...
ii_trial.params.save_chan = save_chan;
...
for tt = 1:ii_cfg.numtrials
    ...
    for chan_idx = 1:length(save_chans)
        ii_trial.(save_chans{chan_idx}){thisidx(tt)} = ii_data.(save_chans{chan_idx})(ii_cfg.trialvec==tt);
    end
    ...
    elseif ~any(this_i_dur<=excl_criteria.i_dur_thresh & this_i_amp>=excl_criteria.i_amp_thresh)  % if none of the saccades pass the amplitude & duration criteria
        ...
        ii_trial.i_sacc_raw(tt,:) = [this_sacc.X_end(this_i_sacc(1)) this_sacc.Y_end(this_i_sacc(1))];
        ii_trial.i_sacc_rt(tt) = this_sacc.t(this_i_sacc(1),1)-t_start;
        ii_trial.i_sacc_trace{tt} = [this_sacc.X_trace{this_i_sacc(1)} this_sacc.Y_trace{this_i_sacc(1)}];
